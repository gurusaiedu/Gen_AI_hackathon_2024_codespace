...
def main():
    ...
    if selected_option == "Image":
        ...
        # history=raw_memory_history['history']
    ...
    # if selected_option == "Multi Module":
    #     if userPromt:
    #         pdf_chain(userPromt)
    #         st.write(pdf_chat_memory())
...
def multiModuleRagProcess(file):  
    if file is not None: 
         ...
         chain_multimodal_rag=controller(file1)
...
def formatAIResponse(raw_memory_history):
    full_string = raw_memory_history['history']  # Get the conversation history string
    human_strings, ai_strings = [], []
    current_speaker = None  # Keep track of the current speaker
    for line in full_string.splitlines():  # Split by newlines
        if line.startswith('Human:'):
            current_speaker = 'Human'
            human_strings.append(line.replace('Human:', '', 1).strip())  # Remove prefix and leading/trailing spaces
        elif line.startswith('AI:'):
            current_speaker = 'AI'
            ai_strings.append(line.replace('AI:', '', 1).strip())  # Remove prefix and leading/trailing spaces
        elif current_speaker:  # Append to the current speaker's list if not a new speaker line
