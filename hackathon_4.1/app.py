...
def main():
    ...
    if raw_memory_history:
        ...
        for idx, chat in enumerate(raw_memory_history):
            if chat.startswith("Human"):
                humanString = chat.replace("Human:", "")
                aiString=raw_memory_history[idx+1].replace("AI:","")
                printUi(humanString,aiString)
...
def printUi(human,AI):
    with st.container():
        st.markdown(f"<div class='question-container'><div class='text-container'><p class='human'>{human}</p class='ai'><p>{AI}</p></div></div>", unsafe_allow_html=True)
